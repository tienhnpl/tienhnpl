...
def get_today_appointments():
    ...
    result = cursor.fetchall()
    print("\n{:<10} {:<5} {:<20} {:<20} {:<8} {:<20} {:<10} {:<10}".format(
        "Address", "No", "Patient Name", "Birthday", "Gender", "Doctor Name", "Status", "Note"))
    print("=" * 100)

    for i, row in enumerate(result, start=1):
        formatted_birthday = row[1].strftime('%Y-%m-%d') if isinstance(row[1], datetime) else row[1]
        print("{:<10} {:<5} {:<20} {:<20} {:<8} {:<20} {:<10} {:<10}".format(
            "Ha Noi", i, row[0], formatted_birthday, row[2], row[3], row[4], "Pending"))
    cursor.close()
    conn.close()
